...
public class TrainUtil {
    ...
    public static Map<String, Boolean> parseSeat(String[] split) {

        String startStr = "";
        for (int i = split.length-2; i >0 ; i--) {
            if(NumberUtils.isParsable(split[i])){
                startStr+= split[i]+" ";
            }
        }

        String seatStrAfter = startStr+split[split.length - 1];
        return parseSeat(seatStrAfter);
    }
